...
public class Gui {
    ...
    public void drawTexturedModalRect(int i, int j, int k, int l, int i1, int j1) {
        ...
        Tessellator tessellator = Tessellator.instance;
        tessellator.startDrawingQuads();
        tessellator.addVertexWithUV(i, j + j1, zLevel, (float) (k) * f, (float) (l + j1) * f1);
        tessellator.addVertexWithUV(i + i1, j + j1, zLevel, (float) (k + i1) * f, (float) (l + j1) * f1);
        tessellator.addVertexWithUV(i + i1, j, zLevel, (float) (k + i1) * f, (float) (l) * f1);
        tessellator.addVertexWithUV(i, j, zLevel, (float) (k) * f, (float) (l) * f1);
        tessellator.draw();
    }
}
